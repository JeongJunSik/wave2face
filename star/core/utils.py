...
def save_image(x, filename, ncol=5 ):
    x = denormalize(x)
    vutils.save_image(x.cpu(), filename, nrow=ncol, padding=0)


@torch.no_grad()
def translate_and_reconstruct(nets, args,  gt_land, gt, gt_mask, prior, filename):
    N, C, H, W = gt.size()
    s = nets.style_encoder( prior )
    x_fake = nets.generator( torch.cat( (gt_land, gt_mask), dim=1 ), s, gt )

    x_concat = [ gt_land, gt, gt_mask, prior,  x_fake ]
    x_concat = torch.cat(x_concat, dim=0)
    # print("x_concat.shape: ", x_concat.shape)
    save_image(x_concat, filename, N)
    del x_concat
...
def debug_image(nets, args, inputs, step):
    #x_src, y_src = inputs.gt, inputs.prior
    ...
    device = inputs.gt.device
    N = inputs.gt.size(0)

    # translate and reconstruct (reference-guided)
    filename = ospj(args.sample_dir, '{0}_validation.jpg'.format(step ))
    translate_and_reconstruct(nets, args, gt_land, gt, gt_mask, prior, filename)
